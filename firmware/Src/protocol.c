...
extern uint8_t light_switch, system_switch, temp_alarm, hum_alarm, bright_value;

extern uint16_t humidity_value, temp_sampling, humidity_sampling, maxhum_set, minihum_set;

extern int16_t temp_current, maxtemp_set, minitemp_set;
...
void all_data_update(void)
{
    ...
    mcu_dp_value_update(DPID_TEMP_SAMPLING,temp_sampling); //VALUE型数据上报;
    mcu_dp_value_update(DPID_HUMIDITY_SAMPLING,humidity_sampling); //VALUE型数据上报;
    mcu_dp_value_update(DPID_MAXTEMP_SET,maxtemp_set); //VALUE型数据上报;
    mcu_dp_value_update(DPID_MINITEMP_SET,minitemp_set); //VALUE型数据上报;
    mcu_dp_value_update(DPID_MAXHUM_SET,maxhum_set); //VALUE型数据上报;
    mcu_dp_value_update(DPID_MINIHUM_SET,minihum_set); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_SWITCH,system_switch); //BOOL型数据上报;
    mcu_dp_enum_update(DPID_LIGHT,light_switch); //枚举型数据上报;
    mcu_dp_value_update(DPID_BRIGHT,bright_value); //VALUE型数据上报;
...
static unsigned char dp_download_temp_sampling_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long temp_sampling_read;
    
    temp_sampling_read = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    
    */
    temp_sampling = temp_sampling_read;
    sampling_led_set();
    updateFlash();
    //处理完DP数据后应有反馈
...
static unsigned char dp_download_humidity_sampling_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long humidity_sampling_read;
    
    humidity_sampling_read = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    
    */
    humidity_sampling = humidity_sampling_read;
    sampling_led_set();
    updateFlash();
    //处理完DP数据后应有反馈
...
static unsigned char dp_download_maxtemp_set_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long maxtemp_set_read;
    
    maxtemp_set_read = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    
    */
    maxtemp_set = maxtemp_set_read;
    updateFlash();
    //处理完DP数据后应有反馈
...
static unsigned char dp_download_minitemp_set_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long minitemp_set_read;
    
    minitemp_set_read = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    
    */
    minitemp_set = minitemp_set_read;
    updateFlash();
    //处理完DP数据后应有反馈
...
static unsigned char dp_download_maxhum_set_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long maxhum_set_read;
    
    maxhum_set_read = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    
    */
    maxhum_set = maxhum_set_read;
    updateFlash();
    //处理完DP数据后应有反馈
...
static unsigned char dp_download_minihum_set_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long minihum_set_read;
    
    minihum_set_read = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    
    */
    minihum_set = minihum_set_read;
    updateFlash();
    //处理完DP数据后应有反馈
...
static unsigned char dp_download_light_handle(const unsigned char value[], unsigned short length)
{
    ...
    light = mcu_get_dp_download_enum(value,length);
    light_set(light);
    updateFlash();
    
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_LIGHT, light);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
